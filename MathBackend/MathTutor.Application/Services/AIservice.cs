    ...
    public class AIservice : IAIservice
    {
        ...
        public async Task<string> EvaluateAnswerAsync(string problem, string userAnswer)
        {
            try
            {
                ...
                // We've moved the quadratic equation handling to the controller level
                // This is intentionally left empty as we're handling special cases in the controller
                ...
                1. For problems with multiple solutions (like quadratic equations), consider the answer correct if the user provides ANY valid solution, even if they don't list all possible solutions.
                2. Accept answers in various formats (e.g., 'x = 3', 'x equals 3', or just '3').
                3. Ignore minor formatting issues or extra whitespace.
                4. For equations, accept equivalent forms of the same answer.
                5. Be lenient with rounding in numerical answers.
        ...
        public async Task<string> GetGuidanceAsync(string problem, string solution, string userAnswer, string question)
        {
            try
            {
                ...
                _logger.LogDebug("Sending guidance prompt to Gemini API: {Prompt}", prompt);

                var result = await _kernel.InvokePromptAsync(prompt, new KernelArguments
                {
                    { "Temperature", 0.5 },
                    { "MaxTokens", 600 }  // Reduced to avoid truncation
                });

                var response = result.GetValue<string>();
                _logger.LogDebug("Received guidance response from Gemini API: {Response}", response);

                if (string.IsNullOrWhiteSpace(response))
                {
                    _logger.LogWarning("Empty response received from Gemini API for guidance");
                ...
                if (!string.IsNullOrWhiteSpace(extractedGuidance))
                {
                    ...
                    return $@"{{
                        ""guidance"": ""{EscapeJsonString(extractedGuidance)}""
                    }}";
                }
                ...
                try
                {
                    ...
                    // Create structured response if guidance property is missing
                    if (!jsonObject.TryGetProperty("guidance", out _))
                    {
                        _logger.LogWarning("Missing 'guidance' property in response");
                        return CreateStructuredGuidance(jsonObject);
                    }
                }
                catch (JsonException ex)
                {
                    _logger.LogWarning(ex, "Invalid JSON received from Gemini API for guidance");
                    return CreateFallbackGuidance();
                }
        ...
        // Helper method to extract guidance text from a potentially invalid JSON response
        private string ExtractGuidanceFromResponse(string response)
        {
            try
            {
                // Try to find the guidance field in the response
                int guidanceIndex = response.IndexOf("\"guidance\":");
                if (guidanceIndex >= 0)
                {
                    // Find the start of the guidance value (after the colon and any whitespace)
                    int valueStart = response.IndexOf("\"", guidanceIndex + 10);
                    if (valueStart >= 0)
                    {
                        // Find the end of the guidance value (the next unescaped quote)
                        int valueEnd = -1;
                        bool escaped = false;
                        ...
                        {
                            return response.Substring(valueStart + 1, valueEnd - valueStart - 1);
                        }
                    }
                }

                // If we can't find a properly formatted guidance field, look for any text that might be useful
                if (response.Contains("{") && response.Contains("}"))
                {
                    ...
                    if (start >= 0 && end > start)
                    {
                        ...
                        try
                        {
                            var jsonObject = JsonSerializer.Deserialize<JsonElement>(jsonPart);
                            if (jsonObject.TryGetProperty("guidance", out var guidanceElement) &&
                                guidanceElement.ValueKind == JsonValueKind.String)
                            {
                                return guidanceElement.GetString() ?? "";
                            }
                        }
                        catch
                        {
                ...
                // If all else fails, just return any text that's not markdown formatting
                if (response.Contains("```"))
                {
                    ...
                    if (codeStart >= 0 && codeEnd > codeStart)
                    {
                        ...
                        return response.Substring(codeEnd + 3).Trim();
                    }
                }

                // If nothing else works, just return the raw response
        ...
        {
            return @"{
                ""guidance"": ""I'm sorry, I couldn't generate specific guidance at this time. Please try rephrasing your question or ask your teacher for assistance with this problem.""
            }";
        }

        private string CreateStructuredGuidance(JsonElement partialJson)
        {
            try
            {
                string guidance = "I'm sorry, I couldn't generate specific guidance at this time.";

                // Try to extract any useful information from the partial JSON
                try
                {
                    foreach (var property in partialJson.EnumerateObject())
                    {
                        if (property.Value.ValueKind == JsonValueKind.String)
                        {
                            // Use any string property as guidance
                            guidance = property.Value.GetString() ?? guidance;
                            break;
                        }
                    }
                }
                ...
                {
                    // If we can't enumerate the properties, try to get the raw text
                    string rawText = partialJson.ToString();
                    if (!string.IsNullOrWhiteSpace(rawText) && rawText.Length > 10)
                    {
                        guidance = rawText.Length > 500 ? rawText.Substring(0, 500) + "..." : rawText;
                    }
                }

                // Ensure the guidance is not too long and is properly escaped
                if (guidance.Length > 1000)
                {
                    guidance = guidance.Substring(0, 1000) + "...";
                }

                return $@"{{
                    ""guidance"": ""{EscapeJsonString(guidance)}""
                }}";
            }
            catch
            {
